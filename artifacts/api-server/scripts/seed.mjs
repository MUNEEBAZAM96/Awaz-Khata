...
// علی is seeded with a non-zero balance so the demo ledger shows a real person entry.
...
// --if-empty: only seed when the ledger is missing or empty (safe for automated
// post-merge runs — never clobbers real user transactions).
if (process.argv.includes("--if-empty")) {
  try {
    const raw = await fs.readFile(dataFile, "utf-8");
    const existing = JSON.parse(raw);
    if (Array.isArray(existing) && existing.length > 0) {
      console.log(`Ledger already has ${existing.length} transactions — skipping seed.`);
      process.exit(0);
    }
  } catch {
    // Missing or unreadable file → proceed with seeding.
  }
}
...
const rows = [
  ...
  [4, 14, 0, 2000, "given", "علی", null, "ادھار"],
  ...
  [0, 14, 0, 3000, "expense", null, "petrol", "پٹرول"],
];
...
console.log("People: احمد (باقی 3000), حسن (برابر), علی (باقی 2000)");
